...
output_folder = 'train-image-decalle'
...
for subfolder in subfolders:
    ...
    for filename in os.listdir(input_subfolder):
        ...
        cv2.imwrite(os.path.join(output_subfolder, f"left_{filename}"), shifted_left)
        
        # Décalage de l'image vers la droite
        shifted_right = np.roll(image, 1, axis=1)
        cv2.imwrite(os.path.join(output_subfolder, f"right_{filename}"), shifted_right)
        
        # Décalage de l'image vers le haut
        shifted_up = np.roll(image, -1, axis=0)
        cv2.imwrite(os.path.join(output_subfolder, f"up_{filename}"), shifted_up)
        
        # Décalage de l'image vers le bas
        shifted_down = np.roll(image, 1, axis=0)
        cv2.imwrite(os.path.join(output_subfolder, f"down_{filename}"), shifted_down)
